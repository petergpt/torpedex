...
```text
Let's play Torpedex! Do not inspect, search for, or use any existing local repos or checkouts. Always start from a fresh temp clone with `tmp="$(mktemp -d /tmp/torpedex-XXXXXX)" && git clone --depth 1 https://github.com/petergpt/torpedex.git "$tmp" && cd "$tmp"`. Then read only `CODEX_INSTRUCTIONS.md` and follow it exactly. Start the server with `npm start`, tell me to open http://127.0.0.1:3197, monitor only `/api/live-view` yourself until `phase=finished` or I say stop, and keep the Captain Feed alive with short real notes via `/api/captain-note`. Never use `/api/state`, `/api/stream`, browser tools, or any other hidden-board surface. Do not run commands against any existing local Torpedex repo. Do not run `ls`, `find`, `git status`, `git fetch`, `git pull`, `npm install`, `npm test`, autoplay code, helper scripts, local heuristics, or external API opponents.
```
