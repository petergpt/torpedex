...
function serializeLiveView(game) {
  const lastHumanMove = serializeLiveMove(game.moveLog.find((entry) => entry.actor === "human") || null);
  const lastAgentMove = serializeLiveMove(game.moveLog.find((entry) => entry.actor === "agent") || null);
  return {
    gameId: game.id,
    boardSize: BOARD_SIZE,
    phase: game.phase,
    ...
    humanShipsRemaining: countRemainingShips(game.human),
    agentShipsRemaining: countRemainingShips(game.agent),
    lastEvent: serializeLiveEvent(game.lastEvent),
    lastHumanMove,
    lastAgentMove,
    lastMove: serializeLiveMove(game.moveLog[0] || null),
    captainFeed: serializeCaptainFeed(game.captainFeed),
    codexShots: serializeShotsAgainstBoard(game.human, { revealShipInfo: false }),
    humanShots: serializeShotsAgainstBoard(game.agent, { revealShipInfo: false }),
  };
}

function serializeLiveMove(move) {
  if (!move) {
    return null;
  }
  ...
  return {
    ...
    title: serializeLiveEvent({
      actor: move.actor,
      kind: move.result === "sunk" ? "sunk" : move.result,
      coord: move.coord,
      shipName: null,
    })?.title || move.title,
    detail: serializeLiveEvent({
      actor: move.actor,
      kind: move.result === "sunk" ? "sunk" : move.result,
      coord: move.coord,
      shipName: null,
...
function serializeLiveEvent(event) {
  if (!event) {
    return null;
  }
  ...
  if (event.kind === "miss") {
    return {
      actor: event.actor,
      kind: event.kind,
      title: event.actor === "human" ? "You missed" : "Codex missed",
      detail: `${event.coord} was open water.`,
      coord: event.coord,
      shipName: null,
    };
  }
  ...
  if (event.kind === "hit") {
    return {
      actor: event.actor,
      kind: event.kind,
      title: event.actor === "human" ? "You scored a hit" : "Codex scored a hit",
      detail: `${event.coord} connected.`,
      coord: event.coord,
      shipName: null,
    };
  }
  ...
  if (event.kind === "sunk") {
    return {
      actor: event.actor,
      kind: event.kind,
      title: event.actor === "human" ? "You sank a ship" : "Codex sank a ship",
      detail: `${event.coord} finished a ship.`,
      coord: event.coord,
      shipName: null,
    };
  }
  ...
  if (event.kind === "win") {
    return {
      ...
      detail:
        event.actor === "human"
          ? `Final strike at ${event.coord}. The enemy fleet is gone.`
          : `Codex finished your fleet with a strike at ${event.coord}.`,
      coord: event.coord,
      shipName: null,
    };
  }
